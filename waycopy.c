...
void
data_source_send(void *data, struct ext_data_control_source_v1 *source, const char *mime_type, int32_t fd)
{
...
main(int argc, char *argv[])
{
	...
	struct ext_data_control_source_v1 *source = ext_data_control_manager_v1_create_data_source(data_control_manager);
	if (source == NULL)
		die("source is null");

	ext_data_control_source_v1_offer(source, options.type);
	ext_data_control_source_v1_add_listener(source, &data_source_listener, NULL);
